...
#			    in the case of multi-rod features.
#                           BLUE - optimized one rod features
#                           CYAN - small features
#                           YELLOW - two rod features
#                           RED - three or more rod features
...
def rectangle_fit(imgName,filename="NONE",scale ="1px",minSize=10,threshold=-10,min_asp_mult = 0.5,max_asp_mult = 1.7,
                  ...
                  display = False,if_arr = False):#add typical length input to use instead of median aspect ratio/length
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    # INITIALIZE
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    for d in data:
		...
		d['color'] = 'blue'
		    ...
		    draw_img_optim.line(find_rect_axis(optim_rect),d['color'],width=2) 
	...
	if (out or display) and 'color' in d:
		...
		if (d['color'] == 'cyan' or d['color'] == 'blue'):
			draw_img_lines.line(find_rect_axis(d['rectangle']),d['color'],width=2)
		else:
	...
	if optim:
	    ...
	    featdata.write("\"Feat Type\",\"Quality\",\"Width\",\"Length\",\"Area\",\"Aspect Ratio\",\"End points\",\"Max Edge Length\",\"Cos^2(PHI)\",\"Centroid (X)\",\"Centroid (Y)\"")
            for d in data:
		try:
                    featdata.write("\n{},{},{},{},{},{},{},{},{},{},{}".format(d["rod_type"],d["quality"],d["width"], d["length"],d['area'],d['aspectratio'],d['end_pnt'],d['max_edge_len'],d['cos2phi'],d['xcent'],d['ycent']))
		except:
		    print "feature data write error"
	metafile.close()
        featdata.close()
    if display:
...
def fit_img_stack(stack_name,scale = "1px",series_units = "V",series_vals = [-1],filename="NONE",minSize=10,
	...
	img_avgs['length_mean'] = np.mean([x["length"] for x in data[i][1] if x["rod_type"] == 1])
	img_avgs['length_median'] = np.median([x["length"] for x in data[i][1] if x["rod_type"] == 1])	
	img_avgs['length_stddev'] = np.std([x["length"] for x in data[i][1] if x["rod_type"] == 1])
    ...
    for x in stack_data:
        if series_vals[0] == -1:
            stackdata.write("\n {}{},{}, {},{},{},{},{},{},{},{},{},{},{}".format(i*10,series_units,data[i][0],x["cos2phi_mean"],x["cos2phi_median"],
                                                                               ...
                                                                               x["asp_rat_stddev"],numreg[i],time_elapsed[i],x["num_1rod"],
    ...
    plt.subplot(211)
    plt.plot(series_vals,[x["cos2phi_mean"] for x in stack_data],"-bo")
    plt.ylabel('Mean Cos^2(PHI)')
    plt.xlabel(x_lab)
    plt.subplot(212)
    plt.plot(series_vals,[x["cos2phi_stddev"] for x in stack_data],"-bo")
    plt.ylabel('Std Dev Cos^2(PHI)')
    plt.xlabel(x_lab)
    plt.savefig(filename+"_cos2phidata.jpg")
    plt.cla()
    plt.clf()

    plt.subplot(211)
    plt.plot(series_vals,[x["asp_rat_mean"] for x in stack_data],"-bo")
    plt.ylabel('Mean Aspect Ratio')
    plt.xlabel(x_lab)
    plt.subplot(212)
    plt.plot(series_vals,[x["asp_rat_stddev"] for x in stack_data],"-bo")
    plt.ylabel('Std Dev Aspect Ratio')
    plt.xlabel(x_lab)
    plt.savefig(filename+"_aspectratioData.jpg")
    plt.cla()
    plt.clf()

    plt.subplot(211)
    plt.plot(series_vals,[x["length_mean"] for x in stack_data],"-bo")
    plt.ylabel('Mean Length')
    plt.xlabel(x_lab)
    plt.subplot(212)
    plt.plot(series_vals,[x["length_stddev"] for x in stack_data],"-bo")
    plt.ylabel('Std Dev Length')
    plt.xlabel(x_lab)
    plt.savefig(filename+"LengthData.jpg")
    plt.cla()
    plt.clf()
    ...
    plt.savefig(filename+"_timeElapsed.jpg")
